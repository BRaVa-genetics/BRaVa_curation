...
main <- function(args)
{
	...
	out_meta_results_dir <- paste0(out_dir, "/gene_biobank_specific/n_cases_", n_cases)
	...
	# And now, do a single biobank meta-analysis.
	for (b in unique(results_dt$biobank)) {
		# Ensure the folder is present
		results_dt_tmp <- results_dt %>% filter(biobank == b)
		system(paste0("mkdir -p ", out_meta_results_dir, "/", b))
		for (phe in phes) {
			for (s in c("ALL", "M", "F")) {
				files_gene <- (results_dt_tmp %>% filter(phenotypeID == phe, sex == s))$filename
				if (length(files_gene) < 1) { 
					cat("The phenotype is not present for:\n")
					cat(phe, s, b, "\n")
					next
				}
				
				out <- paste0(out_meta_results_dir, "/", b, "/", phe, "_", s, "_gene_meta_analysis_", n_cases, "_cutoff.", b, ".tsv.gz")
				if (length(files_gene) == 1) {
					out <- gsub("_gene_meta_analysis_", "_gene_analysis_", out)
					system(paste0("cp ", files_gene, " ", out))
					next
				}
				files_gene <- paste(files_gene, collapse=",")
				
				cat(paste0("carrying out meta-analysis of ", phe, " in ", s, " in ", b, "\n"))
				cat(paste0("\nFiles in the analysis: ",
					paste0(strsplit(files_gene, split=",")[[1]], collapse='\n'), "\n"))
				system(paste(
					"sbatch run_meta_analysis_gcloud_bmrc.sh",
					files_gene, out))
				cat(paste0("submitted meta-analysis of ", phe, ":", b, " completed\n\n"))
			}
		}
	}
}
