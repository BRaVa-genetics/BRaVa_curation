...
# Commented code to be carried out locally and uploaded
...
# # Upload 
# system("scp manuscript_figures/biobank_results_at_sig_genes_specific.tsv.gz qen698@cluster2.bmrc.ox.ac.uk:/well/lindgren/dpalmer/BRaVa_meta-analysis_outputs/biobank_results_at_sig_genes_specific.tsv.gz")

# THIS WILL BE UPDATED: - DONE!
...
dt_meta <- fread("/well/lindgren/dpalmer/BRaVa_meta-analysis_outputs/conditioned_meta_analysis_results.tsv.gz")
# Determine the associations that were significant
sig <- dt_meta %>% filter(Pvalue < (0.05/(20000*3*3*2))) %>% select(Region, phenotypeID, max_MAF, Group, class)

# Non-sig after conditioning
not_sig <- dt_meta %>% 
    filter(Pvalue < (0.05/(20000*3*3*2)) & 
        Pvalue_cond >= (0.05/(20000*3*3*2))) %>% select(Region, phenotypeID, max_MAF, Group, class)

nrow(not_sig)/nrow(sig) # used in the paper to show attrition of sig associations
...
# If needed:
# dt_meta <- data.table(dt_meta)

dt_plot <- copy(dt_meta)

dt_plot[, Pvalue := -log10(Pvalue)]
dt_plot[, Pvalue_cond := -log10(Pvalue_cond)]

pdf("manuscript_figures/Figures/pvalue_vs_pvalue_cond.pdf", width = 4, height = 3)

p <- ggplot(dt_plot, aes(x = Pvalue, y = Pvalue_cond)) +
  ...
  labs(
    ...
    y = expression(-log[10](italic(P)[cond]))
  ) +
...
dt_zoom <- dt_plot[
  Pvalue <= 10 & Pvalue_cond <= 10
]

p_zoom <- ggplot(dt_zoom, aes(x = Pvalue, y = Pvalue_cond)) +
  geom_hex(bins = n_bins) +
  geom_abline(
    slope = 1,
    intercept = 0,
