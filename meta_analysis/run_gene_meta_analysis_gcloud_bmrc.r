...
main <- function(args)
{
	...
	results_dt <- results_dt %>% filter(biobank %in% c("uk-biobank", "all-of-us"))
	# for non-EUR
	for (phe in phes) {
		for (s in c("ALL", "M", "F")) {
			...
			if (length(files_gene) <= 1) { 
				...
				files_gene <- paste(files_gene, collapse=",")
				# Ensure the folder is present
				system(paste0("mkdir -p ", out_meta_results_dir, "/just_uk-biobank_and_all-of-us"))
				out <- paste0(out_meta_results_dir, "/just_uk-biobank_and_all-of-us/", phe, "_", s, "_gene_meta_analysis_", n_cases, "_cutoff.non_EUR.tsv.gz")
				cat(paste0("carrying out meta-analysis of ", phe, " in ", s, " for non-EUR\n"))
				cat(paste0("\nFiles in the analysis: ",
					paste0(strsplit(files_gene, split=",")[[1]], collapse='\n'), "\n"))
				system(paste(
					"sbatch run_meta_analysis_gcloud_bmrc.sh",
					files_gene, out))
				cat(paste0("submitted meta-analysis of ", phe, ": just uk-biobank and all-of-us completed\n\n"))
			}
		}
	}
