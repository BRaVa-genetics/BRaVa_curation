...
make_manhattan_plot = function(contigs, positions, pvals, log_OR=NULL,
    label=NULL, size_by_p=FALSE, buffer=100000000, title='', subtitle='',
    threshold=5, chr_lengths=chr_lengths_38, colour_aes=NULL, log_p_vals=FALSE,
    significance_T=5e-8, ggplot_theme=theme_bw, two_tone=TRUE, by_OR=FALSE,
    colour_1='#2b59a1', colour_2='#5fb756', save_figure=FALSE, file='file_out',
    scaling=1, width=230, height=100, title_size=NULL, minus_log_p_max=NULL,
    print_p=FALSE, loglog=FALSE)
{  
    ...
    dt_plot <- data.table(contig=gsub("chr", "", contigs),
        position=as.integer(positions),
        pval=as.numeric(pvals), labels=label) %>%
    mutate(x = dt_contigs[gsub('X', '23', contig), 'start'] + 
        position + (as.integer(gsub('X', '23', contig))-1)*buffer)

    # Include two tone chromosome plotting
    if (two_tone) {
        dt_plot <- dt_plot %>% 
        mutate(colour=ifelse(
            (as.integer(gsub('X', '23', contig)) %% 2) == 0,
            colour_1, colour_2))
    } else {
    ...
    if (by_OR & !is.null(log_OR)) {
        ...
        dt_plot$colour <- factor(dt_plot$colour,
            levels = c("red", "indianred3", "cornflowerblue", "blue3"))
    }
    ...
    if(!log_p_vals) {
        ...
        if (is.null(label)) {
            dt_plot <- dt_plot %>% dplyr::mutate(y = -log10(pval)) %>% 
            dplyr::select(x, y, colour, contig, position)
        } else {
            dt_plot <- dt_plot %>% dplyr::mutate(y = -log10(pval)) %>% 
            dplyr::select(x, y, colour, labels, contig, position)
        }
    } else {
        dt_plot <- as.data.frame(dt_plot)
        if (is.null(label)) {
            dt_plot <- dt_plot %>% dplyr::mutate(y = pval) %>% 
            dplyr::select(x, y, colour, contig, position)
        } else {
            dt_plot <- dt_plot %>% dplyr::mutate(y = pval) %>%
            dplyr::select(x, y, colour, labels, contig, position)
    ...
    if (size_by_p) {
        dt_plot <- dt_plot %>% dplyr::mutate(size = case_when(
            dt_plot$y > 3 ~ 20,
    ...
    dt_plot <- dt_plot %>% dplyr::mutate(y = transform_y(y))
    breaks_trans <- transform_y(breaks)

    if (size_by_p) {
        p <- ggplot(dt_plot, aes(x=x, y=y, col=colour, size=size)) + 
        geom_point_rast()
    } else {
        p <- ggplot(dt_plot, aes(x=x, y=y, col=colour)) + 
    ...
    p <- p + geom_hline(yintercept=-log10(significance_T), color='#E15759',
        linetype='dashed') + 
        scale_x_continuous(
            breaks=dt_contigs$shifted_position,
            labels=gsub(23, 'X', dt_contigs$contig)) +
    ...
    if (by_OR) { 
        p <- p + scale_colour_manual(name="Odds ratio",
            values = levels(dt_plot$colour),
            labels=c("> 10", "1 - 10", "0.1 - 1", "< 0.1"))
    }

    if (!is.null(label))
        p <- p + geom_label_repel(
            data=subset(dt_plot, y > threshold), size = 5,
            aes(label=labels), color='grey30', box.padding = 0.35,
